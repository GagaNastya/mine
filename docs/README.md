...
В рамках курса осуществляется проектирование решения на основе [постановки задачи от "заказчика"](docs/task.md).
...
# Бизнес-метрики
1. Количество поданных докладов.
2. Количество докладчиков, принявших участие в конференции.
3. Количество зарегистрированных слушателей.
4. Уровень удовлетворенности участников конференции (опросы участников).
5. Уровень удовлетворенности докладчиков (опросы докладчиков).
6. Количество спонсоров и партнеров конференции.
7. Количество просмотров трансляции конференции.
8. Количество вопросов от слушателей, полученных во время конференции.
9. Количество обращений в службу поддержки во время конференции.
10. Количество новых подписчиков на информационные ресурсы после конференции.
...
|--------|------------------------------|-------------------------------------------|
| QR.001 | Производительность |Измерять время отклика системы на запросы пользователей и мониторить уровень загрузки сервера. Для этого можно использовать мониторинг нагрузки на сервер и мониторинг времени отклика. Если время отклика системы превышает установленный порог или уровень загрузки сервера становится критическим, требуется оптимизировать систему, увеличив её ресурсы|
|QR.002|Надежность|Система должна иметь резервное копирование данных для защиты от потери информации. Мониторинг должен включать проверку наличия резервных копий данных|
|QR.003|Информационная безопасность|Система должна иметь механизмы защиты от несанкционированного доступа. Мониторинг должен включать оценку степени защиты системы от внешних угроз|
|QR.004|Удобство использования|Система должна быть простой в использовании для всех категорий пользователей, включая докладчиков, слушателей и технический персонал|
|QR.005|Масштабируемость|Мониторинг масштабируемости системы включает проверку производительности системы при увеличении числа пользователей и объема данных. Также следует оценить возможность добавления новых функций и компонентов системы без значительного влияния на её производительность и работоспособность|
|QR.006|Удобство использования|Для мониторинга удобства использования системы можно использовать тестирование пользовательского интерфейса и опрос пользователей о степени удобства использования системы|
